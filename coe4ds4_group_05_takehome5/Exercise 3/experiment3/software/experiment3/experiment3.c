        ...
        while(1) { 
            ...
            if (PB_irq_data.load_img_flag == 1) {
                ...
                if (status < 0) printf("Error opening file \"%s\"\n", PB_irq_data.filename);
                    ...
                    // take out header from bmp
                for(i=0;i<54;i++){
                	status = sd_card_write(file_handle, bmp_header[i]);
				   ...
				   for (i = 0; i < NUM_LINES/4; i++)
                    ...
                    for (i = 0; i < NUM_LINES; i++) {
                        for (j = 0; j < LINE_LEN; j++) {
                        	if((i>119 && i<360) &&(j>159 && j<480)){ //condition to check middle of screen
                        	...
                        	else{	// black on lcd all other regions
                        		B_vals[j] = 0;
                        		G_vals[j] = 0;
                        		R_vals[j] = 0;
                        	}
                        ...
                        LTC_Write_Image_Line(R_vals, G_vals, B_vals); //write line
                    }
...
//                status = sd_card_write(file_handle, '\n');
                ...
                for (i = 0; i < NUM_LINES; i++) {
                    ...
                    for (j = 0; j < LINE_LEN; j++) {
                    	if(i%2 == 0 && j%2 == 0){ //only write on even rows and columns onto sd card
							IOWR(LED_RED_O_BASE, 0, i*640+j);
